...
const tzEl = $('#timezone'), textEnabledBtn = $('#toggle-text-enabled'), voiceEnabledBtn = $('#toggle-voice-enabled');
...
function renderEditor(){
  ...
  syncSwitch($('#toggle-text-enabled'), !!state.model.general.text_enabled, (v)=> state.model.general.text_enabled=v);
  syncSwitch($('#toggle-voice-enabled'), !!state.model.general.voice_enabled, (v)=> state.model.general.voice_enabled=v);
...
function setSwitch(btn, on){ btn.classList.remove('on','off'); btn.classList.add(on?'on':'off'); btn.textContent = on?'ON':'OFF'; }
...
function drawTimes(){
  ...
  state.model.times.forEach((row,i)=>{
    ...
    sw.onclick=()=>{ row.enabled=!row.enabled; sw.className='switch '+(row.enabled?'on':'off'); sw.textContent=row.enabled?'ON':'OFF'; };
    ...
    const tdMsg=document.createElement('td'); const msg=document.createElement('input'); msg.type='text'; msg.value=row.message||''; msg.placeholder='設定テキストメッセージ'; msg.oninput=(e)=>row.message=e.target.value;
    tdMsg.appendChild(msg); tr.appendChild(tdMsg);

    const tdOps=document.createElement('td'); tdOps.textContent='↑ ↓ －'; tr.appendChild(tdOps);
...
async function loadGuild(id){
  ...
  try{ const data=await api.guild(id); state.current=data; state.model=toModel(data); renderGuildList(); renderEditor(); setStatus(`Loaded ${state.model.name}`); }
  catch(e){ console.error(e); setStatus(`Failed to load ${id}`); }
}

async function bootstrap(){
  try{
    const [g,a]=await Promise.all([api.guilds(), api.audio()]);
    state.guilds=g.guilds||[]; state.audioFiles=a.files||[];
    renderGuildList(); renderAudioList(); setStatus('Ready');
  }catch(e){ console.error(e); setStatus('APIに接続できません'); }
}
bootstrap();
